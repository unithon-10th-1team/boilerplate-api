...
public class QuestionService {
    ...
    private final static Integer LIMIT_COUNT = 5;

    public QuestionsResponse getQuestions() {
        var questions = questionRepository.findAll();
        var questionAnswer = questionAnswerRepository.findAll()
                .stream()
                .map(q -> new QuestionAnswerResponse(
                                q.getId(),
                                q.getQuestionId(),
                                q.getAnswer()
                        )
                ).collect(Collectors.toMap(QuestionAnswerResponse::getId, Function.identity()));

        Collections.shuffle(questions);


        var questionAndAnswerModel = questions.stream()
                .limit(LIMIT_COUNT)
                .map(q ->
                        QuestionResponse.of(
                                q,
                                q.getAnswers().stream().map(questionAnswer::get).collect(Collectors.toList())
                        )
