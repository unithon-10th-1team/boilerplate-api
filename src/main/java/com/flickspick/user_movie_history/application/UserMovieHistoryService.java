...
public class UserMovieHistoryService {
    ...
    public UserMovieHistory getRecentHistory(Long userId) {
        var userMovieHistory = userMovieHistoryRepository.findTopByUserIdOrderByCreatedAtDesc(userId);

        if (userMovieHistory.isEmpty()) {
            return userMovieHistoryRepository.findTop1ByOrderByCreatedAtDesc()
