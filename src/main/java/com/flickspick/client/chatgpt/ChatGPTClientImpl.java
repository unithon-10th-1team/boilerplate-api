...
public class ChatGPTClientImpl implements ChatGPTClient {
    ...
    public ResponseMessage query(List<QuestionModel> answers, List<Long> ottIds) {
        WebClient webClient =
                WebClientFactory.generate(
                        "https://api.openai.com/v1/chat/completions", 20000, 15000L, 15000L);
        ...
        requestMessageList.add(
                new RequestMessage(
                        ChatGPTRole.SYSTEM.getValue(),
                        "You are an OTT movie recommendation expert. You can recommend movies currently available on OTT services like"
                                + ottNames
                                + ". Please specify the OTT service (e.g., Netflix) and the country (e.g., Korea) for the recommendation. Provide your recommendation in the following format: {\\\"title\\\":\\\"xxx\\\", \\\"reason\\\":\\\"xxx\\\", \\\"plot\\\":\\\"xxx\\\", \\\"recommend_type\\\":\\\"[types 중 1개 선택]\\\"}. Please respond only in Korean."));
        requestMessageList.add(
                new RequestMessage(
                        ChatGPTRole.SYSTEM.getValue(),
                        "types: ['코난 꿈나무', '방구석 액션전문가', '4계절 내내 여름 공포중독자', '극 I, 평화주의자', '인간 수도꼭지', '영화계의 설민석', '이별중이신가요?', '벚꽃이 보이시나요?']"));
        ...
        for (QuestionModel answer : answers) {
            ...
            requestMessageList.add(
                    new RequestMessage(ChatGPTRole.USER.getValue(), question.getQuestion()));
            ...
            requestMessageList.add(
                    new RequestMessage(
                            ChatGPTRole.ASSISTANT.getValue(), questionAnswer.getAnswer()));
        }

        ChatGPTQueryRequest chatGPTQueryRequest =
                new ChatGPTQueryRequest("gpt-3.5-turbo", requestMessageList);

        ChatGPTQueryResponse queryResponse =
                webClient
                        .post()
